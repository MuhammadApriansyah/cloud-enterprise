  ...
  return (
    <>
      ...
      <div 
        ...
        className={`fixed inset-0 bg-black/70 backdrop-blur-sm z-[45] transition-opacity duration-500 ${isOpen ? "opacity-100" : "opacity-0 pointer-events-none"}`} 
      />

      {/* Off-Canvas Sidebar Panel */}
      <aside 
        className={`fixed top-0 left-0 h-screen w-[320px] bg-[#030303] border-r border-white/[0.05] z-[50] transform transition-transform duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] flex flex-col shadow-[20px_0_50px_rgba(0,0,0,0.5)] ${isOpen ? "translate-x-0" : "-translate-x-full"}`}
      >
        <div className="h-24 px-8 flex items-center justify-between border-b border-white/[0.04]">
          <div className="flex items-center space-x-4">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-indigo-500 to-blue-500 flex items-center justify-center shadow-[0_0_20px_rgba(99,102,241,0.3)]">
        ...
        <nav className="flex-1 py-8 px-5 flex flex-col space-y-2 overflow-y-auto">
          ...
          <Link
            href="/dashboard"
            onClick={() => setIsOpen(false)}
            className={`flex items-center space-x-4 py-4 px-4 rounded-2xl font-medium transition-all ${
              pathname === "/dashboard"
        ...
        <div className="p-6 border-t border-white/[0.04] space-y-6">
          <div className="flex items-center space-x-4 p-4 bg-white/[0.02] border border-white/[0.05] rounded-2xl">
            <div className="w-10 h-10 rounded-full bg-indigo-500/20 border border-indigo-500/30 flex items-center justify-center text-indigo-400 font-bold">U</div>
            <div className="flex flex-col">
              <span className="text-sm font-bold text-slate-200">Tenant User</span>
              <span className="text-[10px] text-teal-400 tracking-wider">SECURED</span>
            </div>
          </div>
